...
Il existe plusieurs méthodes pour réaliser cette conversion, mais j'utiliserai le *cast*[^castdef] `Number(maVariable)` car c'est la méthode qui me semble être la plus simple et la plus polyvalente (puisqu'il est possible d'afficher des entiers comme des décimaux, et que si le nombre n'est pas valide `NaN` (*NotaNumber*) sera affiché).
```
